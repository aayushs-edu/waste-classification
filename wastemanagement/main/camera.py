...
class MaskDetect(object):
	...
	def get_frame(self):
		...
		for i in classNames: 
			if not self.printed:
				try:
					h = self.waste_disposal_advice[i]
					print(f'<p class="card-text" style="font-size: 3vh; color: white; text-align: left;"><b>{i.capitalize()}</b></p>')
					print(f'<p class="card-text" style="font-size: 2vh; color: white; text-align: left;">{self.waste_disposal_advice[i]}</p>')
				except: pass
		self.printed = True
		# coordinates
		for r in results:
